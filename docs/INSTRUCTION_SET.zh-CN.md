...
所有 16-bit 字按 big-endian 存储：高字节位于低地址。例如指令字 `5028h` 在 `.bin` 和内存中占用 `50h 28h`。未特别说明时，16-bit 整数运算按模 `65536` 回绕，不更新 `SR`。
...
| `008h` | `LDW Rd, [p20]` | 从物理地址读一个 big-endian 字。 |
| `009h` | `LSTB Rs, [p20]` | 向物理地址写 `Rs` 的低字节。 |
| `00Ah` | `LSTW Rs, [p20]` | 向物理地址写一个 big-endian 字。 |
